SELECT DISTINCT
    CASE
        WHEN t.TagName = 'chromedriver' THEN 'selenium'
        WHEN t.TagName = 'selenium-chromedriver' THEN 'selenium'
        WHEN t.TagName = 'selenium-webdriver' THEN 'selenium'
        WHEN t.TagName = 'webdriver' THEN 'selenium'
        ELSE t.TagName
    END AS tag,
...
WHERE p.PostTypeId = 1
  AND p.CreationDate >= '2023-04-29'
  AND p.CreationDate < '2023-05-01'
  AND t.TagName IN (
    ...
    'xcode','xml','chromedriver','selenium-chromedriver','selenium-webdriver','webdriver'
  )
ORDER BY
    creation_date,
    question_id,
